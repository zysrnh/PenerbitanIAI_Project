...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Admin Panel'); ?> | IAI PERSIS PRESS</title>
    ...
    <aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-slate-300 flex flex-col justify-between transition-transform duration-300 transform -translate-x-full lg:translate-x-0 border-r border-slate-800">
        <div>
            ...
            <nav class="p-3 space-y-1 text-xs">
                ...
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg font-semibold transition <?php echo e(request()->routeIs('admin.dashboard') ? 'bg-emerald-500/10 text-emerald-400 font-bold' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'); ?>">
                    <i class="fa-solid fa-chart-pie w-4 text-center text-sm"></i>
                    <span>Dashboard</span>
                </a>

                <a href="<?php echo e(route('admin.messages.index')); ?>" class="flex items-center justify-between px-3 py-2 rounded-lg font-semibold transition <?php echo e(request()->routeIs('admin.messages.*') ? 'bg-emerald-500/10 text-emerald-400 font-bold' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'); ?>">
                    ...
                    <?php if($unreadCount > 0): ?>
                        <span class="px-1.5 py-0.2 rounded-full text-[9px] font-extrabold bg-amber-500 text-slate-950"><?php echo e($unreadCount); ?></span>
                    <?php endif; ?>
                </a>

                <a href="<?php echo e(route('admin.users.index')); ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg font-semibold transition <?php echo e(request()->routeIs('admin.users.*') ? 'bg-emerald-500/10 text-emerald-400 font-bold' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'); ?>">
                ...
                <a href="<?php echo e(route('admin.settings.contact')); ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg font-semibold transition <?php echo e(request()->routeIs('admin.settings.*') ? 'bg-emerald-500/10 text-emerald-400 font-bold' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200'); ?>">
    ...
    <div class="flex-1 lg:pl-64 flex flex-col min-h-screen">
        ...
        <header class="h-16 bg-white border-b border-slate-200/80 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-30">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-sm sm:text-base font-bold text-slate-900 leading-tight"><?php echo $__env->yieldContent('header_title', 'Dashboard'); ?></h2>
                </div>
            </div>

            <div class="flex items-center gap-3">
                <a href="<?php echo e(url('/')); ?>" target="_blank" class="px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg border border-slate-200 transition flex items-center gap-1.5">
        ...
        <footer class="py-4 px-8 border-t border-slate-200/80 text-center text-xs text-slate-400 bg-white">
            &copy; <?php echo e(date('Y')); ?> IAI PERSIS PRESS &bull; Sistem Manajemen Penerbitan
        </footer>
    </div>
...
<?php /**PATH D:\Gawe\IAIPersis\penerbitan.iaibandung.ac.id\resources\views/admin/layouts/app.blade.php ENDPATH**/ ?>