...
<head>
    ...
    <title>Pesanan Telah Dikirim #{{ $order->order_number }}</title>
</head>
                    ...
                    <tr>
                        <td style="padding: 24px 36px 12px;">
                            <div style="background-color: #f0fdf4; border-left: 4px solid #006830; padding: 14px 18px; border: 1px solid #bbf7d0; border-left-width: 4px;">
                                <div style="font-size: 11px; font-weight: 800; color: #166534; text-transform: uppercase; letter-spacing: 0.5px;">STATUS PENGIRIMAN PESANAN</div>
                                <div style="font-size: 16px; font-weight: 800; color: #064e3b; margin-top: 2px;">
                                    🚚 Pesanan Buku Anda Telah Dikirimkan
                                </div>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding: 12px 36px 28px;">
                            <p style="font-size: 14px; line-height: 1.7; color: #334155; margin-top: 0;">
                                ...
                                Paket pesanan buku Anda untuk nomor invoice <strong>#{{ $order->order_number }}</strong> telah selesai dipacking dan saat ini sudah diserahkan ke pihak jasa ekspedisi/kurir untuk diantar ke alamat Anda.
                            </p>

                            <!-- Tracking Info Box -->
                            <table width="100%" border="0" cellspacing="0" cellpadding="10" style="background-color: #f8fafc; border: 1px solid #e2e8f0; font-size: 13px; margin: 18px 0;">
                                <tr>
                                    <td width="35%" style="color: #64748b; font-weight: bold; border-bottom: 1px solid #e2e8f0;">Nomor Resi / Ekspedisi</td>
                                    <td style="color: #006830; font-weight: 900; font-family: monospace; font-size: 15px; border-bottom: 1px solid #e2e8f0;">
                                        {{ $order->tracking_number ?: 'Dalam Proses Pengantaran Kurir' }}
                                    </td>
                                </tr>
                                <tr>
                                    <td style="color: #64748b; font-weight: bold; border-bottom: 1px solid #e2e8f0;">Status Pengiriman</td>
                                    <td style="color: #0f172a; font-weight: bold; text-transform: capitalize; border-bottom: 1px solid #e2e8f0;">
                                        {{ str_replace('_', ' ', $order->shipping_status) }}
                                    </td>
                                </tr>
                                <tr>
                                    <td style="color: #64748b; font-weight: bold; vertical-align: top; border-bottom: 1px solid #e2e8f0;">Alamat Pengiriman</td>
                                    <td style="color: #334155; line-height: 1.5; border-bottom: 1px solid #e2e8f0;">
                                        {{ $order->customer_address }}
                                    </td>
                                </tr>
                                <tr>
                                    <td style="color: #64748b; font-weight: bold;">Penerima &amp; WhatsApp</td>
                            ...
                            <table width="100%" border="0" cellspacing="0" cellpadding="8" style="border: 1px solid #e2e8f0; font-size: 12.5px; margin: 18px 0;">
                                <tr style="background-color: #f8fafc;">
                                    <th align="left" style="font-weight: 800; color: #0f172a; border-bottom: 1px solid #e2e8f0;">Daftar Buku Dikirim</th>
                                    <th align="center" width="60" style="font-weight: 800; color: #0f172a; border-bottom: 1px solid #e2e8f0;">Qty</th>
                                </tr>
                                @foreach($items as $item)
                                <tr>
                                    <td style="color: #0f172a; font-weight: bold; border-bottom: 1px solid #f1f5f9;">{{ $item['title'] ?? 'Buku PERSIS PERS' }}</td>
                                    <td align="center" style="color: #475569; border-bottom: 1px solid #f1f5f9;">{{ $item['quantity'] ?? ($item['qty'] ?? 1) }} eks</td>
                                </tr>
                                @endforeach
                            </table>

                            <p style="font-size: 13px; line-height: 1.6; color: #475569;">
                                Anda dapat memantau status pesanan kapan saja melalui akun member atau membuka invoice resmi di bawah ini.
                            </p>
                            ...
                            <div style="text-align: center; margin-top: 24px;">
                                ...
                                @php
                                    ...
                                    $waLink = "https://wa.me/{$waNumber}?text=" . urlencode("Halo Tim Redaksi PERSIS PERS, saya ingin menanyakan pengiriman pesanan #" . $order->order_number);
                                @endphp
                                <a href="{{ $waLink }}" target="_blank" style="background-color: #25D366; color: #ffffff; padding: 12px 20px; text-decoration: none; font-size: 13px; font-weight: bold; display: inline-block;">
                                    💬 Hubungi WhatsApp Redaksi
                                </a>
                            </div>
