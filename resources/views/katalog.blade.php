...
@section('content')
    <!-- Flat Minimalist Header (Solid Flat Brand-950, Zero Gradients, Sharp Corners) -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 animate-fade-in-up">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div class="w-full md:w-80 shrink-0">
                    <div class="relative">
                        <input type="text" id="catalogSearch" placeholder="Cari judul, penulis, ISBN..." class="w-full pl-9 pr-4 py-2 rounded bg-[#064e3b] border border-emerald-700/60 text-white placeholder-emerald-200 text-xs focus:outline-hidden focus:ring-1 focus:ring-emerald-400" />
    ...
    <!-- 4 Flat Quick Highlight Cards (Zero Gradients, Crisp Sharp Borders) -->
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-20">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            <!-- Card 1 -->
            <div class="bg-white p-4 rounded border border-slate-200 shadow-xs flex items-center gap-3">
                <div class="w-9 h-9 rounded bg-emerald-50 text-[#006830] flex items-center justify-center text-sm shrink-0 border border-emerald-100">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <aside class="lg:col-span-3 space-y-4">
                    ...
                    <!-- Box 1: Kategori Buku -->
                    <div class="bg-white rounded border border-slate-200 shadow-xs overflow-hidden">
                        <div class="bg-[#032c21] px-3.5 py-2.5 text-white flex items-center justify-between">
                            ...
                            <span class="text-[10px] bg-[#064e3b] text-emerald-300 font-bold px-1.5 py-0.5 rounded border border-emerald-800">24</span>
                        </div>
                        <div class="p-1 divide-y divide-slate-100 text-xs">
                            @foreach($categories as $cat)
                                <button type="button" class="w-full flex items-center justify-between px-3 py-2 rounded font-medium text-slate-700 hover:bg-slate-100 hover:text-[#006830] transition text-left group {{ $cat['slug'] === 'all' ? 'bg-emerald-50 text-[#006830] font-bold' : '' }}">
                                    <span class="flex items-center gap-1.5">
                                        <i class="fa-solid fa-angle-right text-[9px] text-slate-400 group-hover:text-[#006830]"></i>
                                        {{ $cat['name'] }}
                                    </span>
                                    <span class="text-[10px] font-bold text-slate-400 group-hover:text-[#006830] bg-slate-100 px-1.5 py-0.5 rounded">
                                        {{ $cat['count'] }}
                                    </span>
                                </button>
                            @endforeach
                    ...
                    <div class="bg-white rounded border border-slate-200 shadow-xs p-3.5 space-y-2.5">
                    ...
                    <div class="bg-white rounded border border-slate-200 shadow-xs overflow-hidden">
                        <div class="bg-[#b45309] px-3.5 py-2.5 text-white">
                            <span class="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
                    ...
                    <div class="bg-[#032c21] rounded p-4 text-white border border-[#064e3b] space-y-2">
                        ...
                        <a href="{{ route('kontak') }}" class="block text-center py-2 px-3 bg-[#006830] hover:bg-[#005226] text-white font-bold text-xs rounded transition">
                ...
                <main class="lg:col-span-9 space-y-7">
                    ...
                    <div class="space-y-3">
                        <!-- Solid Header Bar (No Gradient) -->
                        <div class="bg-[#006830] px-4 py-2.5 rounded text-white flex items-center justify-between shadow-xs">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3.5">
                            @foreach($newBooks as $book)
                                <div class="bg-white rounded border border-slate-200 shadow-xs hover:border-[#006830] transition flex flex-col justify-between p-3">
                                    
                                    <div>
                                        <!-- Solid Flat Book Cover (Rectangular, Sharp, No Gradient) -->
                                        <div class="relative h-52 rounded-xs bg-[#032c21] text-white p-3.5 flex flex-col justify-between border-l-4 border-[#006830] shadow-xs">
                                            <!-- Top Category Tag -->
                                            <div class="flex items-center justify-between">
                                                <span class="text-[9px] font-bold uppercase bg-[#064e3b] text-emerald-300 px-1.5 py-0.5 rounded-xs">
                                                    {{ $book['category'] }}
                                                </span>
                                            ...
                                            <!-- Footer Author & ISBN -->
                                            <div class="pt-2 border-t border-[#064e3b] text-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-2 grid grid-cols-2 gap-1.5">
                                            <button type="button" onclick="openBookModal({{ json_encode($book) }})" class="w-full py-1.5 px-2 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-bold text-center transition flex items-center justify-center gap-1">
                                                <i class="fa-solid fa-eye text-[10px] text-slate-500"></i> Detail
                                            </button>
                                            <a href="https://wa.me/6282116116133?text={{ urlencode('Halo Redaksi PERSIS PERS, saya ingin memesan buku: ' . $book['title']) }}" target="_blank" class="w-full py-1.5 px-2 rounded bg-[#006830] hover:bg-[#005226] text-white text-[11px] font-bold text-center transition flex items-center justify-center gap-1 shadow-xs">
                    ...
                    <div class="space-y-3 pt-2">
                        <!-- Solid Header Bar (Amber Solid, No Gradient) -->
                        <div class="bg-[#b45309] px-4 py-2.5 rounded text-white flex items-center justify-between shadow-xs">
                            <h2 class="font-extrabold text-sm sm:text-base font-heading tracking-wide uppercase flex items-center gap-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3.5">
                            @foreach($bestSellers as $book)
                                <div class="bg-white rounded border border-slate-200 shadow-xs hover:border-amber-600 transition flex flex-col justify-between p-3">
                                    
                                    <div>
                                        <!-- Solid Flat Book Cover -->
                                        <div class="relative h-52 rounded-xs bg-[#032c21] text-white p-3.5 flex flex-col justify-between border-l-4 border-amber-500 shadow-xs">
                                            <!-- Top Category Tag -->
                                            <div class="flex items-center justify-between">
                                                <span class="text-[9px] font-bold uppercase bg-[#064e3b] text-amber-300 px-1.5 py-0.5 rounded-xs">
                                                    {{ $book['category'] }}
                                                </span>
                    ...
                    <div class="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-600">
                        ...
                        <div class="flex items-center gap-1 font-bold">
                            <button class="px-2.5 py-1 rounded bg-[#006830] text-white">1</button>
                            <button class="px-2.5 py-1 rounded bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 transition">2</button>
                            <button class="px-2.5 py-1 rounded bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 transition">3</button>
                            <button class="px-2.5 py-1 rounded bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 transition">Berikutnya &rarr;</button>
                        </div>
                    </div>
    ...
    <!-- Modal Quick-View Detail Buku (100% Solid Flat Colors, Sharp Minimalist Corners) -->
    <div id="bookModal" class="fixed inset-0 z-50 bg-black/60 hidden items-center justify-center p-3 sm:p-4 overflow-y-auto">
        <div class="bg-white rounded-md max-w-2xl w-full shadow-2xl border border-slate-300 overflow-hidden relative animate-fade-in-up my-6">
            
            <!-- Modal Header (Solid Flat #032c21, Zero Gradients) -->
            <div class="bg-[#032c21] text-white px-5 py-3 flex items-center justify-between border-b border-[#064e3b]">
                ...
                <button onclick="closeBookModal()" class="w-7 h-7 rounded bg-[#064e3b] hover:bg-[#08634c] text-slate-200 hover:text-white flex items-center justify-center text-xs font-bold transition">
            ...
            <div class="p-5 sm:p-6">
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-5 items-start">
                    ...
                    <!-- Left: Solid Flat Book Cover (Rectangular, Sharp) -->
                    <div class="sm:col-span-5 flex flex-col items-center">
                        <div class="relative w-full h-64 rounded-xs bg-[#032c21] text-white p-4 flex flex-col justify-between border-l-4 border-[#006830] border border-slate-800 shadow-sm">
                            <div class="flex justify-between items-center">
                                <span id="modalCategoryBadge" class="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-xs bg-[#064e3b] text-emerald-300 border border-emerald-700/50"></span>
                                <span class="text-[9px] text-slate-400 font-mono">PERSIS PERS</span>
                            </div>
                        ...
                        <!-- Solid Price Box -->
                        <div class="mt-3 w-full bg-slate-50 border border-slate-200 rounded p-2.5 text-center">
                    ...
                    <div class="sm:col-span-7 space-y-3.5">
                        ...
                        <!-- Technical Specs Grid (Solid Flat) -->
                        <div class="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-2.5 rounded border border-slate-200">
                            <div>
                        ...
                        <div>
                            ...
                            <div id="tabContentCitation" class="hidden pt-2">
                                <div class="bg-slate-100 p-2 rounded text-[10.5px] text-slate-700 font-mono border border-slate-200">
                                    <span id="modalCitationText"></span>
                                </div>
                            </div>
                        </div>

                        <!-- Action Buttons (Solid Flat Colors, Sharp Rectangles) -->
                        <div class="pt-2.5 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <a id="modalWaBtn" href="#" target="_blank" class="w-full py-2 px-3 rounded bg-[#006830] hover:bg-[#005226] text-white font-bold text-xs flex items-center justify-center gap-1.5 transition">
                                <i class="fa-brands fa-whatsapp"></i> Pesan via WhatsApp
                            </a>
                            <button type="button" onclick="alert('Sampel Bab 1 & Daftar Isi PDF siap diunduh!')" class="w-full py-2 px-3 rounded bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs flex items-center justify-center gap-1.5 transition border border-slate-200">
